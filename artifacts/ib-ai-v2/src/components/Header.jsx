...
function exportChat(messages, title, format) {
  ...
  const safeTitle = (title || 'ib-ai-chat').replace(/[^a-z0-9_\-\s]/gi, '').trim().replace(/\s+/g, '-').toLowerCase() || 'ib-ai-chat';

  let content;
  if (format === 'md') {
    const date = new Date().toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric' });
    content = `# ${title || 'IB AI Chat'}\n_Exported on ${date}_\n\n---\n\n` +
      messages.map(m => `**${m.role === 'user' ? 'You' : 'IB AI'}**:\n${m.content}`).join('\n\n---\n\n');
  } else {
    content = messages.map(m => `${m.role === 'user' ? 'You' : 'IB AI'}: ${m.content}`).join('\n\n');
  }
